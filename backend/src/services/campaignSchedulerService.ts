...
class CampaignSchedulerService {
  ...
  private async processNextMessage(campaign: any, message: any) {
    ...
    try {
      ...
      if (campaign.messageType === 'ai_agent') {
        // Lógica para o Agente de IA (n8n)
        const tenantSettings = await prisma.tenantSettings.findUnique({ where: { tenantId: campaign.tenantId } });
        
        if (tenantSettings?.aiAgentEnabled) {
          console.log(`🤖 Iniciando Agente de IA (n8n) para contato ${message.contactPhone}`);
          const n8nResult = await n8nService.sendAiAgentWebhook(campaign.tenantId, contact, campaign);
          
          if (n8nResult.success) {
            result = { success: true, messageId: 'AI_AGENT_STARTED' }; // Simular sucesso
          } else {
            result = { success: false, error: n8nResult.message };
          }
        } else {
          result = { success: false, error: 'Agente de IA desabilitado para este tenant.' };
        }
      ...
      // Se for AI_AGENT, o messageId é simulado, mas o fluxo de sucesso é o mesmo
      if (campaign.messageType === 'ai_agent' && result.success) {
        // Não há necessidade de atualizar o sentCount, pois não é uma mensagem enviada
        // Apenas marca como SENT para que o fluxo da campanha continue
        await prisma.campaignMessage.update({
          where: { id: message.id },
          data: {
            status: 'SENT',
        ...
        console.log(`AI Agent webhook sent successfully to ${message.contactPhone}`);
        return;
      }
