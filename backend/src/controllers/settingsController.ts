...
export const settingsValidation = [
  ...
  body('n8nWebhookUrl').optional().custom((value) => {
    if (!value || value === '') return true;
    if (!/^https?:\/\/.+/.test(value)) {
      throw new Error('Webhook URL do n8n deve ser uma URL válida');
    }
    return true;
  }),
  body('n8nApiKey').optional().custom((value) => {
    if (!value || value === '') return true;
    if (value.length < 10) {
      throw new Error('API Key do n8n deve ter pelo menos 10 caracteres');
    }
    return true;
  })
];
...
export const getSettings = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const combinedSettings = {
      ...
      n8nWebhookUrl: tenantSettings?.n8nWebhookUrl || null,
      n8nApiKey: tenantSettings?.n8nApiKey || null,
      aiAgentEnabled: tenantSettings?.aiAgentEnabled || false
    };
...
export const updateSettings = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { wahaHost, wahaApiKey, evolutionHost, evolutionApiKey, quepasaUrl, quepasaLogin, quepasaPassword, companyName, pageTitle, openaiApiKey, groqApiKey, chatwootUrl, chatwootAccountId, chatwootApiToken, n8nWebhookUrl, n8nApiKey, aiAgentEnabled, tenantId } = req.body;
    ...
    // Atualizar configurações do tenant (APIs de IA, Chatwoot e n8n)
    let tenantSettings = null;
    if (effectiveTenantId && (openaiApiKey !== undefined || groqApiKey !== undefined || chatwootUrl !== undefined || chatwootAccountId !== undefined || chatwootApiToken !== undefined || n8nWebhookUrl !== undefined || n8nApiKey !== undefined || aiAgentEnabled !== undefined)) {
